...
// GET: SMS Server pulls pending scheduled/treatment messages or updates status
export async function GET(request) {
  try {
    ...
    // Filter pending scheduled/treatment messages scheduled for now or in the past
    const now = new Date();
    const pendingSms = smsQueue
      .filter(sms => {
        if (sms.status !== 'pending') return false;
        if (sms.type !== 'scheduled') return false;
