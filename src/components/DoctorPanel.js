      ...
      <div className="flex justify-end gap-3 mt-6 no-print">
        ...
        {prescriptions.length > 0 && (
          <button className="btn btn-outline" onClick={() => {
            ...
            const html = `<!DOCTYPE html><html><head><title>Retsept</title><style>@page{size:80mm auto;margin:0}body{font-family:'Courier New',monospace;width:68mm;margin:0 auto;padding:8px 2px;font-size:11px;color:#000}.header{font-size:14px;font-weight:bold;text-align:center;text-transform:uppercase;margin-bottom:2px}.divider{border-top:1px dashed #000;margin:6px 0}table{width:100%;border-collapse:collapse;font-size:10px}th,td{border-bottom:1px dotted #ccc;padding:3px 2px;text-align:left}th{font-size:9px;text-transform:uppercase;color:#555}.footer{font-size:9px;text-align:center;margin-top:10px;font-style:italic}</style></head><body><div class="header">KLINIKA CRM</div><div style="text-align:center;font-size:10px">RETSEPT</div><div class="divider"></div><div style="font-size:10px;margin-bottom:4px"><b>Bemor:</b> ${patient.lastName} ${patient.firstName}<br><b>Yosh:</b> ${getAge(patient.birthDate)} | <b>ID:</b> ${patient.id}<br><b>Shifokor:</b> ${docName} (${docSpec})<br><b>Sana:</b> ${new Date().toLocaleDateString('uz-UZ')}</div><div class="divider"></div><table><thead><tr><th>#</th><th>Dori</th><th>Doza</th><th>Qabul</th><th>Muddat</th><th>Ko'rsatma</th></tr></thead><tbody>${rxRows}</tbody></table><div class="divider"></div><div class="footer">Sog'ligingiz — bizning baxtimiz!<br>Ushbu retsept faqat shifokor tomonidan beriladi.</div><script>window.onload=function(){window.print();setTimeout(function(){window.close()},300)};</script></body></html>`;
